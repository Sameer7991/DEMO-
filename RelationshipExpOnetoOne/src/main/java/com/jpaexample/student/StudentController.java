...
public class StudentController {
    ...
    @PostMapping
    public ResponseEntity<Address> createStudent(@RequestBody Address student) {
        Address createdStudent = studentService.createStudent(student);
        return new ResponseEntity<>(createdStudent, HttpStatus.CREATED);
    }
    
    @GetMapping
    public ResponseEntity<List<Address>> getAllStudents() {
        List<Address> students = studentService.getAllStudents();
        return new ResponseEntity<>(students, HttpStatus.OK);
    }

    // Get a student by ID
    @GetMapping("/{id}")
    public ResponseEntity<Address> getStudent(@PathVariable Long id) {
        Address student = studentService.getStudentById(id);
